 ...
 * @requires ForceWebsite, QRCode
 * @note: make sure that content element has ID "content"
 *        so that ForceWebsite can detect the element
 *        using ForceWebsite.onContentReady
 */
...
this.init=function(plug){
  ...
  plug.Force.loadScriptFile(
    (plug.hosts.hasOwnProperty('social')
      ?plug.hosts.social
      :plug.root)
    +'/social/qrcode.min.js');
  const _social=this;
  ForceWebsite.onContentReady(r=>{
    ...
    qrc=ForceWebsite.buildElement('div',null,{
      id:'qrcode',
    }),
